...
object Newcsvconnectionrandomforest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val spark = SparkSession.builder()
      .appName("Create DataFrame Random Forest from CSV file")
      .master("local[*]")
...
//    val users_pipe_read3_renamed = users_pipe_read3.withColumnRenamed("branch_id", "b_id")
    ...
    //    val sumValue = users_pipe_read1.agg(sum("total_price")).first().getLong(0)
    //    val totalunitValue = users_pipe_read1.agg(sum("quantity_sold")).first().getLong(0)

//    var joinedDF = users_pipe_read1.join(users_pipe_read3_renamed, users_pipe_read1("shoe_id") === users_pipe_read3_renamed("shoe_id"), "full_outer")
      var joinedDF = users_pipe_read1.join(users_pipe_read2, users_pipe_read1("shoe_id") === users_pipe_read2("shoe_id"), "inner")
    //    joinedDF  = users_pipe_read2.crossJoin(joinedDF).where(col("shoe_id") === col("shoe_id"))
    ...
    // Print the sum
    //    println(s"Total amount: $sumValue")
    //    println(s"Total unit sold: $totalunitValue")
    ...
    val model = pipeline.fit(joinedDF)
    val indexedDF = model.transform(joinedDF)
    indexedDF.show()
